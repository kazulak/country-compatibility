...
def run_tests():
    ...
    if len(unis) != 10:
        print(f"FAIL: Expected 10 universities, found {len(unis)}")
        sys.exit(1)
    else:
        print(f"PASS: Verified 10 universities in academic db.")

    if len(groups) != 10:
        print(f"FAIL: Expected 10 groups, found {len(groups)}")
        sys.exit(1)
    else:
        print(f"PASS: Verified 10 research groups in academic db.")
